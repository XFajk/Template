...
/// <summary>
/// A reusable 2D trigger zone that detects when a PhysicsBody2D enters the area,
/// emits a signal, and optionally disables itself after triggering or for a cooldown period.
/// </summary>
[GlobalClass]
public partial class Trigger2D : Area2D
{
    /// <summary>
    /// Emitted when a valid PhysicsBody2D enters the trigger area.
    /// </summary>
    [Signal]
    public delegate void TriggeredEventHandler(PhysicsBody2D body);

    /// <summary>
    /// A list of PhysicsBody2D nodes that are allowed to trigger this area.
    /// If empty, any body is allowed to trigger it.
    /// </summary>
    [Export]
    public Array<PhysicsBody2D> TriggerNodes;
    ...
    {
        PhysicsBody2D body = node as PhysicsBody2D;
        if (body == null)
            return;
